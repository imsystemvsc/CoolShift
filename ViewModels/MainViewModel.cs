    ...
    public bool StartWithWindows
    {
        ...
        set
        {
            try
            {
                ...
                if (value)
                {
                    key?.SetValue(AppName, $"\"{Environment.ProcessPath}\" --hidden");
                }
                else
                {
            ...
            {
                System.Windows.MessageBox.Show($"Failed to update startup settings: {ex.Message}", "Error", System.Windows.MessageBoxButton.OK, System.Windows.MessageBoxImage.Error);
            }
        }
    }
    ...
    public MainViewModel()
    {
        ...
        _temperatureSeries = new ISeries[]
        {
            new LineSeries<ObservableValue>
            {
                ...
                Fill = new SolidColorPaint(new SKColor(247, 178, 103, 50)),
                Stroke = new SolidColorPaint(new SKColor(247, 178, 103)) { StrokeThickness = 2 },
                GeometryFill = null,
                GeometryStroke = null,
                GeometrySize = 0,
                LineSmoothness = 0.8
